...
{
    // 获得成员变量
    NSArray *cachedProperties = nil;
    @try{
        NSString*class = NSStringFromClass([self class]);
        if(class){
            cachedProperties = [[DBTypeMangeShare sharedDBMange].enumeratePropertiesDict objectForKey:class];
        }
        if(!cachedProperties){
            cachedProperties = [self properties];
            if(cachedProperties){
                [[DBTypeMangeShare sharedDBMange].enumeratePropertiesDict setObject:cachedProperties forKey:class];
            }
        }
    }@catch(NSException* e) {
        cachedProperties = @[];
